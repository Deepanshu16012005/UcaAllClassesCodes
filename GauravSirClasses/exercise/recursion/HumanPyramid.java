...
public class HumanPyramid {
     ...
     */
    public float totalWeightSupported(List<List<Float>> pyramid, int row, int col) {
      if(row == 0){
        return pyramid.get(row).get(0);
      }
      // khud da wieght + left + right
      float left = 0;
      if(col - 1 >= 0){
      	left = totalWeightSupported(pyramid , row - 1, col - 1)/2;
      }

      float right = 0;
      if(pyramid.get(row-1).size() > col){
        right = totalWeightSupported(pyramid,row - 1, col)/2;
      }

      return pyramid.get(row).get(col) + left + right;
    }
    ...
    public static void main(String[] args) {
        ...
        List<List<Float>> pyramid = Arrays.asList(
            Arrays.asList(51.18f),
            Arrays.asList(55.90f, 131.25f),
            Arrays.asList(69.05f, 133.66f, 132.82f),
            Arrays.asList(53.43f, 139.61f, 134.06f, 121.63f)
        );
